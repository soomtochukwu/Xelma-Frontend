...
const getErrorMessage = (
  error: Extract<ValidationResult, { valid: false }>["error"],
  net: Network,
): string => {
  switch (error) {
    case "invalid-format":
    ...
    case "wrong-network":
      return `Address doesn't match selected network. Please use a ${net === "MAINNET" ? "Mainnet" : "Testnet"} address.`;
    case "not-found":
...
export function useStellarAddressValidation(
  ...
  const validate = useCallback(async (addr: string, net: Network) => {
    ...
    if (cached) {
      ...
      if (age < CACHE_TTL && cached.network === net) {
        ...
        if (result.valid) {
          ...
          setErrorMessage(getErrorMessage(result.error, net));
        }
        return;
      } else {
